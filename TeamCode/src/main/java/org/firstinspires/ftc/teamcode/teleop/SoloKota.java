    ...
    public void loop() {
        ...
        if (gamepad1.b){
            CommandScheduler.getInstance().schedule(new SequentialCommandGroup(
                    new InstantCommand(() -> IntakeSubsystem.getInstance().setTarget(0)),
                    new InstantCommand(() -> IntakeSubsystem.getInstance().bucketUp()),
                    new WaitCommand(500),
                    new InstantCommand(() -> IntakeSubsystem.getInstance().slurpStop()),
                    new WaitCommand(50),
                    new InstantCommand(() -> IntakeSubsystem.getInstance().doorOpen())
            ));
            iTarget = 0;
        }
        ...
        if(gamepad1.left_stick_button) {
            CommandScheduler.getInstance().schedule(new SequentialCommandGroup(
                    new InstantCommand(() -> IntakeSubsystem.getInstance().bucketNeutral()),
                    new InstantCommand(() -> OuttakeSubsystem.getInstance().setArmState(OuttakeArmState.downToTransfer)),
                    new WaitCommand(75),
                    new InstantCommand(() -> OuttakeSubsystem.getInstance().setClawOpen(false))
            ));
            grab = true;
        }
